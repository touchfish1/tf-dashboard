  ...
  return (
    <View style={{ height }}>
      <Svg width="100%" height={h} viewBox={`0 0 ${w} ${h}`}>
        ...
        <Path d={areaPath} fill={CHART_COLORS.secondary} fillOpacity={0.1} />
        <Path d={linePath} stroke={CHART_COLORS.secondary} strokeWidth={2.5} fill="none" strokeLinecap="round" strokeLinejoin="round" />
        {data.map((d, i) => (
          <Circle key={i} cx={x(i)} cy={y(d.balance)} r={12} fill="transparent"
        ...
        {selData && (
          <G>
            ...
            <Circle cx={x(sel as number)} cy={y(selData.balance)} r={5} fill={CHART_COLORS.secondary} />
          </G>
        )}
      </Svg>
      {selData && (
        <View className="absolute top-1 left-0 right-0 items-center" pointerEvents="none">
          <View className="bg-zinc-800 dark:bg-zinc-100 px-3 py-1.5 rounded-card-sm">
            <Text className="text-xs font-mono text-white dark:text-zinc-900 font-semibold">¥{selData.balance.toFixed(2)}</Text>
